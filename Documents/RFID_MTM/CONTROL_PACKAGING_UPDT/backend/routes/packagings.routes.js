...
router.post("/", auth, (req, res) => {
  const { packaging_name, packaging_type_id, warna } = req.body;

  if (!packaging_name || !packaging_type_id || !warna) {
    return res.status(400).json({ message: "Data tidak lengkap" });
  }

  db.query(
    `INSERT INTO packagings
      (packaging_name, packaging_type_id, warna, created_at)
     VALUES (?, ?, ?, NOW())`,
    [packaging_name, packaging_type_id, warna],
    (err) => {
      if (err)
...
router.put("/:id", auth, (req, res) => {
  ...
  db.query(
    `UPDATE packagings SET
      ...
      warna = ?
     WHERE id = ?`,
    [packaging_name, packaging_type_id, warna, req.params.id],
    (err) => {
      if (err)
