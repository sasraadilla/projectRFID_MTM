...
const Forecast = () => {
  ...
  const loadForecasts = async () => {
    try {
      const res = await api.get("/forecasts"); // BE endpoint
      // res.data sudah berupa array forecast dengan join ke customer
  ...
  const handleDelete = async (id) => {
    const result = await Swal.fire({
      title: "Yakin hapus?",
      text: "Forecast dan detailnya akan dihapus permanen",
      icon: "warning",
  ...
  // Filter search berdasarkan nama customer atau bulan/tahun
